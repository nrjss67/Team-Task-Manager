...
from task_manager.forms import AddComentaryForm, ProjectCreateForm, ReplyMassageForm, SignUpForm, TaskCreateForm
...
class IndexView(View):
    def get(self, request, *args, **kwargs):
        if request.user.is_authenticated:
            ...
            return redirect(reverse_lazy("task_manager:home_page", kwargs={"team_name": team_name}))
...
class PersonalTaskListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        context = Task.objects.filter(assignees__username__icontains=self.request.user.username)
...
class TeamProjectListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["status"] = self.request.GET.get("status", "all")
        context["priority"] = self.request.GET.get("priority", "all")
        context["completed_projects"] = Project.objects.filter(is_complited=True).count()
...
class TeamProjectTaskListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_context_data(self):
        ...
        context["completed_tasks"] = Project.objects.get(pk=self.kwargs["pk_project"]).tasks.filter(is_complited=True).count()
        context["not_completed_tasks"] = Project.objects.get(pk=self.kwargs["pk_project"]).tasks.filter(is_complited=False).count()
        context["high_priority_tasks"] = Project.objects.get(pk=self.kwargs["pk_project"]).tasks.filter(priority="high").count()
        return context
...
class CreateProjectView(LoginRequiredMixin, generic.CreateView):
    ...
    def get_success_url(self):
        team_name = get_slugify_team_name(self.request)["team_name"]
        return reverse("task_manager:team_projects_list", kwargs={"team_name": team_name})
...
class CreateTaskView(LoginRequiredMixin, generic.CreateView):
    ...
    def get_success_url(self):
        ...
        return reverse("task_manager:team_project_tasks_list", kwargs={"team_name": team_name, "pk_project": project_pk})
...
class UpdateTaskView(LoginRequiredMixin, generic.UpdateView):
    ...
    def get_context_data(self, **kwargs):
        context = super().get_context_data(**kwargs)
        context["team_members"] = Task.objects.get(pk=self.kwargs["pk_task"]).project.team.worker_set.all()
        return context
...
class InboxListView(LoginRequiredMixin, generic.ListView):
    ...
    def get_queryset(self):
        queryset = MessageNew.objects.filter(to_worker=self.request.user.id, is_archieve=False)
    ...
    def post(self, request, *args, **kwargs):
        ...
        if action == "archive":
            MessageNew.objects.filter(id__in=message_ids, to_worker=request.user.id).update(is_archieve=True)
        elif action == "delete":
            MessageNew.objects.filter(id__in=message_ids, to_worker=request.user.id).delete()
...
class MessageDetailView(LoginRequiredMixin, generic.DetailView, generic.edit.FormMixin):
    ...
    def get_form_kwargs(self):
        kwargs = super().get_form_kwargs()
        kwargs.update({"request": self.request, "to_worker": self.get_queryset()[0].from_worker})
        return kwargs
